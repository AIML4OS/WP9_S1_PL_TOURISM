...
Tourismset is a synthetic version of the original Tourismset due to disclosure control. Synthetic version was generated with `synthesizer` package available on [CRAN](https://CRAN.R-project.org/package=synthesizer).

First, we load the Tourismset.
...
Check structure of the Tourismset
...
- `period_yyyymm` - text label of year and month of reporting (convenient for Tourism ordering)
...
In this section we explore our Tourism through several plots. First, we start with histogram but on log scale.

```{r}
library(scales)
Tourism %>% filter(tourists>0) %>% 
...
Tourism %>% filter(voivodeship %in% top_voiv) %>% 
